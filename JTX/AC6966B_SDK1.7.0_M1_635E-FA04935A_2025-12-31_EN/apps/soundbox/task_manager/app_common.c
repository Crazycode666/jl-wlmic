...
extern bool line_mode_flag;

extern int disk_flag;

int rec_turn_music = 0;

bool is_rec_mode = false;
    ...
    case KEY_CHANGE_MODE:
...

        {
            ...
            /*gpio_set_direction(IO_PORTA_00,0);
                ...
                gpio_set_output_value(IO_PORTA_00,1);
                */
            extern int disk_flag;
            //disk_flag = 0;
            rec_turn_music = 0;
    ...
    case KEY_ENC_START:
       // extern void user_recoder();
        ...
        if(disk_flag)
        {
            ...
            //app_music_task();
//            gpio_set_direction(IO_PORTA_00,0);
//            gpio_set_output_value(IO_PORTA_00,0);
...
static u8 power_on_falg_wait_sd = TRUE;
static u8 power_on_falg_wait_usb = TRUE;
u8 switch_sd_mode_falg = FALSE;

static void wait_sd_usb_online_function(void)
{
    power_on_falg_wait_sd = FALSE;
    power_on_falg_wait_usb = FALSE;
    if(!app_check_curr_task(APP_MUSIC_TASK))
    {
        printf("find_sd_no_online\n");
    ...
    case DRIVER_EVENT_FROM_SD2:
        ...
        if (ret == true) {
            if (event->u.dev.event == DEVICE_EVENT_IN) {
                ...
                #if 0
                    if (app_get_curr_task() != APP_MUSIC_TASK) {
                        if(((u32)event->arg == DEVICE_EVENT_FROM_OTG) && (power_on_falg_wait_sd == TRUE))
                        {
                            power_on_falg_wait_sd = FALSE;
                            if(power_on_falg_wait_usb == TRUE)
                            {
                                printf("wait_sd_online \n");
                                sys_timeout_add(NULL,wait_sd_usb_online_function,500);
                            }
                            else{
                                app = APP_MUSIC_TASK;
                            }
                        }
                        else if(((u32)event->arg != DEVICE_EVENT_FROM_OTG) && (power_on_falg_wait_usb == TRUE))
                        {
                            power_on_falg_wait_usb = FALSE;
                            if(power_on_falg_wait_sd == TRUE)
                            {
                                printf("wait_usb_online \n");
                                sys_timeout_add(NULL,wait_sd_usb_online_function,500);
                            }
                            else{
                                app = APP_MUSIC_TASK;
                            ...
                            if(((u32)event->arg != DEVICE_EVENT_FROM_OTG) && (dev_manager_get_total(1) > 1))
                            {
                                switch_sd_mode_falg = TRUE;
                            }
                            app = APP_MUSIC_TASK;
                        }
