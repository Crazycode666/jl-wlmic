...
#define CIG_CONNECTED_TIMEOUT   (0 * 1000L)  //单位:ms
...
struct app_cig_conn_info {
    ...
    struct app_cis_conn_info cis_conn_info[LEA_CIG_CONNECTION_NUM];
};
...
static void identification_record_handler(u16 acl_hdl, u8 arg_num, int *argv)
{
    ...
    if (trans_role == CONNECTED_SYNC_CALL_RX) {
        for (i = 0; i < CIG_MAX_NUMS; i++) {
            for (j = 0; j < LEA_CIG_CONNECTION_NUM; j++) {
...
extern void audio_mic_pwr_io(u32 gpio, u8 out_en);


extern bool trans_pp;
...
void app_connected_conn_status_event_handler(struct wireless_trans_event *wireless_trans)
{
    ...
    case CIG_EVENT_CENTRAL_CONNECT:
        ...
        cis_connected_nums++;
        ASSERT(cis_connected_nums <= LEA_CIG_CONNECTION_NUM && cis_connected_nums >= 0, "cis_connected_nums:%d", cis_connected_nums);
    ...
    case CIG_EVENT_ACL_CONNECT:
        g_printf("CIG_EVENT_ACL_CONNECT");
         ui_user_mode_set(STATUS_BT_INIT_OK, PWM_LED1_ON, 0);
        ...
        acl_connected_nums++;
        ASSERT(acl_connected_nums <= LEA_CIG_CONNECTION_NUM && acl_connected_nums >= 0, "acl_connected_nums:%d", acl_connected_nums);
    ...
    case CIG_EVENT_ACL_DISCONNECT:
        g_printf("CIG_EVENT_ACL_DISCONNECT");
         ui_user_mode_set(STATUS_BT_INIT_OK, PWM_LED0_ON, 0);
        acl_info = (cis_acl_info_t *)wireless_trans->value;
        if (acl_info->conn_type) {
...
void app_connected_close(u8 cig_hdl, u8 status)
{
    ...
    if (find) {
        ...
        acl_connected_nums -= LEA_CIG_CONNECTION_NUM;
        ASSERT(acl_connected_nums <= LEA_CIG_CONNECTION_NUM && acl_connected_nums >= 0, "acl_connected_nums:%d", acl_connected_nums);
        cis_connected_nums -= LEA_CIG_CONNECTION_NUM;
        ASSERT(cis_connected_nums <= LEA_CIG_CONNECTION_NUM && cis_connected_nums >= 0, "cis_connected_nums:%d", cis_connected_nums);
        if (cig_connection_timeout) {
            sys_timeout_del(cig_connection_timeout);
            cig_connection_timeout = 0;
...
#elif (CIG_TRANSPORT_MODE == CIG_MODE_DUPLEX)
    if ((connected_role == CONNECTED_ROLE_CENTRAL) ||
...
void app_connected_remove_pairs_rx_addr(void)
{
     u32 pair_bind_code = 0;
    u8 status = app_get_connected_role();
    if (status) {
        app_connected_close_all(APP_CONNECTED_STATUS_STOP);
    }

    int ret = syscfg_write(VM_WIRELESS_PAIR_RX_BIND_CODE, &pair_bind_code, sizeof(pair_bind_code));
    if (status) {
        app_connected_open(1);
    }
}
...
void my_broadcast_custom_data(u8 type, u16 value)
{
    u8 sync_data[2];
...
//    connected_send_data_to_sibling(CONNECTED_DATA_SYNC_FUNC_ID, &sync_data, sizeof(sync_data), CONNECTED_DEVICE_IDENTIFICATION_L);
    connected_send_data_to_sibling(CONNECTED_DATA_SYNC_FUNC_ID, &sync_data, sizeof(sync_data), CONNECTED_DEVICE_IDENTIFICATION_M);
