...
{
    app_audio_set_volume(APP_AUDIO_STATE_MUSIC, 14, 1);
}
...
void app_audio_volume_up(u8 value)
{
    ...
    case APP_AUDIO_STATE_MUSIC:
    case APP_AUDIO_STATE_LINEIN:
        printf("********* # 222222222222222 app_var.music_volume = %d #**************\n",app_var.music_volume);
        if(app_var.music_volume < 16)
            app_var.music_volume += value;
        else{
            my_broadcast_custom_data(160,1);
        }
        if (app_var.music_volume > get_max_sys_vol()) {
            app_var.music_volume = get_max_sys_vol();
        }
...
#if 0//(SYS_VOL_TYPE ==VOL_TYPE_DIGGROUP)
