...
void noisegate_parm_default_init()
{
    ...
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        ...
        mic_eff->noise_gate_parm.parm.attackTime = 50;
        mic_eff->noise_gate_parm.parm.releaseTime = 5;
        mic_eff->noise_gate_parm.parm.threshold = -42300;//mdb -90.3dB
        mic_eff->noise_gate_parm.parm.low_th_gain = 1;
    }
#endif
}
...
void mic_gain_parm_default_init()
{
    ...
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        ...
        mic_eff->gain_parm.parm.gain[0] = 0;
        mic_eff->gain_parm.parm.gain[1] = 0;
        mic_eff->gain_parm.parm.gain[2] = 0;
        mic_eff->gain_parm.parm.gain[3] = 0;
    }
#endif
}
...
void mic_gain_file_analyze_init()
{
    ...
    for (int index = 0 ; index < EFFECT_REVERB_PARM_MAX; index++) {
        ...
        void *tar_buf = (void *)&mic_eff->gain_parm;
        u16 tar_len = sizeof(mic_eff->gain_parm);
        eff_file_analyze(eff_mode_seq[index], EFF_MIC_GAIN, tar_buf, tar_len);
#endif
    }
#endif
}
