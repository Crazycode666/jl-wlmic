...
#define DEMO_MIC_PCM_BUF_LEN		(4 *1024)
...
#define DEMO_ENC_FRAME_LEN		(2 * 1024) //根据不同的编码每次读取的长度不同进行调整
...
int demo_enc_open(int sample_rate, u8 ch_num, u32 code_type, u32 code_param)
{
    ...
    case AUDIO_CODING_JLA:
        fmt.bit_rate = JLA_CODING_BIT_RATE;
...
static int demo_dec_dec_start(void)
{
    ...
    u8  dec_out_ch_num = audio_decoder_get_channel(&dec->decoder);
    // 配置mixer通道参数
    ...
    /* printf("dec_out_ch_num = %d,ch_num = %d\n",dec_out_ch_num,ch_num); */
    if (dec_out_ch_num != ch_num) {
        if (dec_out_ch_num == 1) {
            audio_mixer_ch_set_aud_ch_out(&dec->mix_ch, 0, BIT(0) | BIT(1)); //单变双
        } else {
            audio_mixer_ch_set_aud_ch_out(&dec->mix_ch, 0, BIT(0));       //双变单
            audio_mixer_ch_set_aud_ch_out(&dec->mix_ch, 1, BIT(0));
        }
    }
#endif
    // 数据流串联
...
int demo_dec_open(int sample_rate, u8 ch_num, u32 code_type, u32 code_param)
{
    ...
    case AUDIO_CODING_JLA:
        ...
        dec->in_fmt.bit_rate = JLA_CODING_BIT_RATE;
...
// mic enc to dec
//-----------------------------------------------------------------------------
//mic 单声道编解码测试 直接调用这个接口即可
//jla参数直接在板级修改即可，speex和opus默认8000采样率。
