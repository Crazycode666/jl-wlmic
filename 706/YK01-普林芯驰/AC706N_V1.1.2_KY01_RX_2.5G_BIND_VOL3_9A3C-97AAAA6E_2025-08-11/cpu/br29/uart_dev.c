...
#define UART_OT_CLK  UART_CLK


static uart_bus_t uart_t[2];
...
/* _WEAK_ */
/* extern */
const u32 CONFIG_UART0_ENABLE_TX_DMA =  0;//uart0 has no dma.
...
void uart_dma_rx_reset(uart_bus_t *ut)
{
    ((JL_UART_TypeDef *)ut->argv)->CON1 |= BIT(4);
    ut->kfifo.buf_in = 0;
    ut->kfifo.buf_out = 0;
    ((JL_UART_TypeDef *)ut->argv)->RXSADR = (u32)ut->kfifo.buffer;
    ((JL_UART_TypeDef *)ut->argv)->RXEADR = (u32)(ut->kfifo.buffer + ut->kfifo.buf_size);
    ((JL_UART_TypeDef *)ut->argv)->RXCNT = ut->frame_length;
    ((JL_UART_TypeDef *)ut->argv)->CON1 &= ~BIT(4);
}

u8 tx_rx_pin[2] = {0xff, 0xff};
...
//校验中断 parity
    if ((JL_UART0->CON2 & BIT(6)) && (JL_UART0->CON2 & BIT(8))) {
        ...
        printf("err:-------uart0 parity fail! RB8:%d,rx num:%d\n", !!(JL_UART0->CON2 & BIT(2)), rx_err_len);
    }
}
/**
 * @brief ut0接收字符串
...
static void UT0_write_buf(const u8 *buf, u32 len)
{
    ...
    if (CONFIG_UART0_ENABLE_TX_DMA) { //have no dma
        uart_set_dma_dir(0, 0);
...
static void UT0_set_baud(u32 baud)
{
    ...
    JL_UART0->BAUD = (UART_CLK / baud) / 4 - 1;
...
static void UT0_open(u32 baud, void *cbuf, u32 cbuf_size, u32 rx_cnt, u32 ot, u32 is_9bit, enum uart_parity_type parity_type)
{
    ...
    /* } */
    /* JL_UART1->CON1 |= BIT(5);//rx bypass en */
    if (is_9bit) {
        /* SFR(JL_UART0->CON0, 18, 2, 2); //滤波 */
        /* SFR(JL_UART0->CON0, 16, 2, 2); //滤波 */
...
//校验中断 parity
    if ((JL_UART1->CON2 & BIT(6)) && (JL_UART1->CON2 & BIT(8))) {
        ...
        printf("err:-------uart1 parity fail! RB8:%d,rx num:%d\n", !!(JL_UART1->CON2 & BIT(2)), rx_err_len);
    }
}
/**
 * @brief ut1接收字符串
...
static u8 ut_tx_pin[] = {
    IO_PORTA_05, IO_PORTB_00, IO_PORTP_00, IO_PORTC_03, //UT0_TX
    /* IO_PORTB_06, IO_PORTC_01, IO_PORTA_00, IO_PORT_DP,  //UT1_TX */
};
static u8 ut_rx_pin[] = {
    IO_PORTA_06, IO_PORTB_01, IO_PORTP_00, IO_PORTC_03, //UT0_RX
...
const uart_bus_t *uart_dev_open(const struct uart_platform_data_t *arg)
{
    ...
    //优先指定串口
    if ((arg->argv == JL_UART0 || arg->argv == JL_UART1) && (!(((JL_UART_TypeDef *)arg->argv)->CON0 & BIT(0)))) {
        ut_num = 0;
        if (arg->argv == JL_UART1) {
            ut_num = 1;
        }
    }
    if (arg->rx_cbuf) {
    ...
    if (CONFIG_UART0_ENABLE && ut_num == 0) {
        ...
        if (ut_ch > 3) {
            if (arg->tx_pin != (u8) - 1) {
                ...
                gpio_och_sel_output_signal(arg->tx_pin, OUTPUT_CH_SIGNAL_UART0_TX);
            }
            if (arg->rx_pin != (u8) - 1) {
                gpio_set_direction(arg->rx_pin, 1);
                gpio_set_die(arg->rx_pin, 1);
                gpio_set_pull_up(arg->rx_pin, 1);
                gpio_ich_sel_iutput_signal(arg->rx_pin, INPUT_CH_SIGNAL_UART0_RX, INPUT_CH_TYPE_GP_ICH);
            }
        }
...
u32 uart_dev_close(uart_bus_t *ut)//return 0:ok, 1:err
{
    ...
    if (&uart_t[0] == ut) {
        ...
        tx_rx_pin[1] = 0xff;
        return 0;
        /* return uart_gpio_close(const struct uart_platform_data_t *arg, 0, PFI_UART1_RX) */
    }
    return 1;
}
...
static u8 _rts_io = -1;
static u8 _cts_io = -1;
#define CTS_INV_SET_HIGH 1//1:high; 0:low
#define RTS_INV_SET_HIGH 1//1:high; 0:low
void uart1_flow_ctl_init(u8 rts_io, u8 cts_io)
{
    ...
    if (cts_io < IO_PORT_MAX) {
        ...
        /* if(cts_io != IO_PORTA_03){ */
        gpio_ich_sel_iutput_signal(cts_io, INPUT_CH_SIGNAL_UART1_CTS, INPUT_CH_TYPE_GP_ICH);
        /* } */
        JL_UART1->CON1 |= BIT(3);//CTS 中断 en
...
#if 0//uart0
//polling byte test
    arg.tx_pin = IO_PORTB_00;//不使用发送功能赋值0xff
    arg.rx_pin = IO_PORTB_01;//不使接收功能赋值0xff
    ...
    arg.baud = 115200;//1000000;//波特率
    printf("----uartx test!----\n");
    if (uart_dev_close(&uart_t[0])) {
        printf("err:~~~~~~~~~uartx close fail!~~~~~~~~~~\n");
    }
    ut = uart_dev_open(&arg);
    if (ut == 0) {
        printf("err:------uartx init fail!--------\n");
    } else {
        printf("------uartx init ok!--------\n");
        printf("uartx: con0:0x%x,con1:0x%x,con2:0x%x\n", JL_UART0->CON0, JL_UART0->CON1, JL_UART0->CON2);
        printf("uartx: baud:0x%x,buf:0x%x\n", JL_UART0->BAUD, JL_UART0->BUF);
        /* printf("uartx: RX_ERR_CNT:0x%x\n", JL_UART0->RX_ERR_CNT); */
        ...
        while (rx_cnt) {
            if (ut->getbyte(&read_data[0], time_out)) {
                ...
                printf("receive 1 byte fail!please send 1 byte!\n");
            }
        }
        time_out = 9000;
        rx_cnt = 5;
        while (rx_cnt) {
            ...
            if (rx_len) {
                ...
                printf("receive buf fail!please send some characters!\n");
            }
        }
    }
#else//uart1
//ie dma test
    ...
    arg.tx_pin = IO_PORTA_00; //不使用发送功能赋值0xff
    arg.rx_pin = IO_PORTA_01; //不使接收功能赋值0xff
    arg.rx_cbuf = read_buf;//en dma, en rx/ot ie
    arg.rx_cbuf_size = sizeof(read_buf);
    arg.frame_length = 44;//54,64,74.dma接收长(<=sizeof(read_buf))
    arg.rx_timeout = 50;//单位ms
    arg.isr_cbfun = NULL; //中断回调
    arg.argv = JL_UART1;
    arg.parity_type = UT_PARITY_EVEN;//UT_PARITY_EVEN;
    arg.is_9bit = 0;//校验使能, 默认关闭滤波
    arg.baud = 1000000;//波特率
    printf("----uartx test!----");
    if (uart_dev_close(&uart_t[1])) {
        printf("err:~~~~~~~~~uartx close fail!~~~~~~~~~~\n");
    }
    ut = uart_dev_open(&arg);
    if (ut == 0) {
        ...
        printf("------uartx init ok!--------uart1:0x%x\n", JL_UART1->CON0);
        //发送
        printf("uartx: con0:0x%x,con1:0x%x,con2:0x%x\n", JL_UART1->CON0, JL_UART1->CON1, JL_UART1->CON2);
        printf("uartx: baud:0x%x,buf:0x%x\n", JL_UART1->BAUD, JL_UART1->BUF);
        printf("uartx: RX_ERR_CNT:0x%x\n", JL_UART1->RX_ERR_CNT);
        //发送
        ut->putbyte(write_buf[0]);
        ut->putbyte('\n');
