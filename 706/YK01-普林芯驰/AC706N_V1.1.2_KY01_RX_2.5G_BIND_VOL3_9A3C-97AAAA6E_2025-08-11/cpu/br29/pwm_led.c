...
extern void pwm_led_open();
extern void pwm_led_close();

extern void ui_user_mode_set(u8 status, u8 display, u8 para_sel);

void echo_led_on(u32 gpio_port,u8 sw)
{
    pwm_led_open(sw);
...
void echo_led_off(u32 gpio_port,u8 sw)
{
    pwm_led_close(sw);
    gpio_set_direction(gpio_port, 0);
...
void led_flash()
{
    //echo_led_on(TCFG_PWMLED_PIN1,1);
    echo_led_on(TCFG_PWMLED_PIN,0);
