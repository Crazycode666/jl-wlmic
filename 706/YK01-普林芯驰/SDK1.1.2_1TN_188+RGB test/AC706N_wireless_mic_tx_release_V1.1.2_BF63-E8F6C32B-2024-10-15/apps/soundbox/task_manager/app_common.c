...
extern void user_vbat_display_del();
extern void user_vbat_display_add();
extern void really_vbat_display();
...
static u16 vbat_clock_display_time = 0;

void vbat_clock_display_add()
{
    if(!vbat_clock_display_time)
        vbat_clock_display_time = sys_timeout_add(NULL,vbat_clock_display,8000);
}

void vbat_clock_display_del()
{
    if(vbat_clock_display_time)
    {
        sys_timeout_del(vbat_clock_display_time);
        vbat_clock_display_time = 0;
    }
}

static u16 recover_display_time = 0;
void recover_display_add()
{
    if(!recover_display_time)
        recover_display_time = sys_timeout_add(NULL,recover_display,3000);
}


void recover_display_del()
{
    if(recover_display_time)
    {
        sys_timeout_del(recover_display_time);
        recover_display_time = 0;
    }
...
int app_common_key_msg_deal(struct sys_event *event)
{
    ...
    case KEY_DISPLAY_VBAT:
        vbat_clock_display_del();
        led_water_flash_add();
