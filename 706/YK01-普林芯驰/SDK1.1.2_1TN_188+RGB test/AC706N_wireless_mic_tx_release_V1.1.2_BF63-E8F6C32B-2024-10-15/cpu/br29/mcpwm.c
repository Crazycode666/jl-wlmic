...
void user_pwm_close(u16 pwm_ch)
{
    mcpwm_close(pwm_ch);
}
...
void mcpwm_test(void)
{
    ...
    pwm_p_data.h_pin = IO_PORTB_06;                         //任意IO，不需要则填-1
    pwm_p_data.h_duty = 5000;                               //占空比，选的引脚有值才有效
    pwm_p_data.l_pin = IO_PORTB_07;                         //任意IO，不需要就填-1
...
void user_pwm_led1()
{
    ...
    pwm_p_data.pwm_ch_num = pwm_ch0;                        //通道号
    pwm_p_data.frequency = 10000;                           //10KHz
    pwm_p_data.h_pin = IO_PORTB_00;                                  //任意IO，不需要则填-1
    pwm_p_data.h_duty = 2000;                               //占空比，选的引脚有值才有效
    pwm_p_data.l_pin = IO_PORTB_00;                         //任意IO，不需要就填-1
    pwm_p_data.l_duty = 2000;                               //占空比，选的引脚有值才有效
    mcpwm_init(&pwm_p_data);
#endif
