...
#define led_debug(fmt, ...) 	g_printf("[PWM_LED] " fmt "\t|\tfun:%s: line:%d", ##__VA_ARGS__, __FUNCTION__, __LINE__)
#define led_err(fmt, ...) 		r_printf("[PWM_LED_ERR] " fmt "\t|\tfun:%s: line:%d", ##__VA_ARGS__, __FUNCTION__, __LINE__)
#else
#define led_debug(...)
#define led_err(...)
#endif

#define led_info printf
...
/* #define PWM0_RC_CLK32K_VALUE 	(__get_lrc_hz() / 4 + 11000) */
#define PWM0_RC_CLK32K_VALUE 	(__get_lrc_hz() / 4 + 9000)
...
static void pwm_clock_set(u8 _clock)
{
    ...
    case PWM_LED_CLK_RC32K:
        ...
        SFR(JL_PLED->CON0, 4, 4, 0b0101);//div8  ==> 25K
        /* SFR(JL_PLED->CON0, 4, 4, 0b0100);//div16  ==> 50K */
        SFR(JL_PLED->CON0, 2, 2, 0b00);
        break;
    case PWM_LED_CLK_BTOSC_24M:
        SFR(JL_PLED->CON0, 2, 2, 0b10);
        break;
    default:
        break;
    }
...
void pwm_led_init(const struct led_platform_data *user_data)
{
    ...
    led_debug("pwm_led_init_version_v1.2");
...
void gpio_led_high_block(u32 gpio_port)
{
    gpio_set_pull_up(gpio_port, 0);
    gpio_direction_input(gpio_port);
    gpio_set_pull_down(gpio_port, 0);
    gpio_set_output_value(gpio_port, 0);
    gpio_set_die(gpio_port, 0);
}
...
#define   LED_188_IO_1  IO_PORTA_04//PORT_PIN_1
#define   LED_188_IO_2  IO_PORTC_02//PORT_PIN_1
#define   LED_188_IO_3  IO_PORTC_03//PORT_PIN_2
#define   LED_188_IO_4  IO_PORTC_04//PORT_PIN_3
#define   LED_188_IO_5  IO_PORTC_05//PORT_PIN_4
...
u8 tens = 8; 		//十位，10-F，11-不显示
u8 unit = 8;		//个位，10-F，11-不显示

void all_led_gpio_off()
{
    gpio_led_high_block(LED_188_IO_1);
    gpio_led_high_block(LED_188_IO_2);
    gpio_led_high_block(LED_188_IO_3);
    gpio_led_high_block(LED_188_IO_4);
    gpio_led_high_block(LED_188_IO_5);
}


void Display_Scan1(void)
{
    echo_led_off(LED_188_IO_1);				//拉低Pin1
    if(nums[unit] & BIT(5))	//B3
    	echo_led_on(LED_188_IO_2);
    if(nums[unit] & BIT(3))	//D3
		echo_led_on(LED_188_IO_3);
	if(nums[unit] & BIT(1))	//F3
		echo_led_on(LED_188_IO_4);
	if(nums[unit] & BIT(0))	//G3
		echo_led_on(LED_188_IO_5);
}

//2脚拉低，可显示A3 B2 D2 E2
void Display_Scan2(void)
{
    echo_led_off(LED_188_IO_2);
    if(nums[unit] & BIT(6))	//A3
    	echo_led_on(LED_188_IO_1);
