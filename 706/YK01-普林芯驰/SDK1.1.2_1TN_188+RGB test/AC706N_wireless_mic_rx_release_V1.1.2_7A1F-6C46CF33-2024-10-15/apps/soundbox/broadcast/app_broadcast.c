...
//bis监听超时后回到配对模式功能使能
#define BIS_CATCH_TIMEOUT_ENABLE    1
//bis监听超时时间
#define BIS_CATCH_TIME              (10 * 1000L)
...
int broadcast_custom_data_deal(void *priv);
...
static int bis_catch_timeout = 0;   /*!< bis监听超时变量，超时后回到配对模式 */
...
extern void echo_led_on(u32 gpio_port);
void app_broadcast_conn_status_event_handler(struct wireless_trans_event *wireless_trans)
{
...
#if (BROADCAST_ENTER_PAIR_BEFORE_RUN && BIS_CATCH_TIMEOUT_ENABLE)
        if (bis_catch_timeout) {
            sys_timeout_del(bis_catch_timeout);
            bis_catch_timeout = 0;
        }
#endif
    ...
    case BIG_EVENT_RECEIVER_DISCONNECT:
     ...
     app_broadcast_close(APP_BROADCAST_STATUS_STOP);
     //app_big_hdl_info[0].used = 0
     app_broadcast_open();

        break;

    case BIG_EVENT_PERIODIC_DISCONNECT:
        g_printf("BIG_EVENT_PERIODIC_DISCONNECT");
        break;

    case BIG_EVENT_CUSTOM_DATA_SYNC:
        g_printf("BIG_EVENT_CUSTOM_DATA_SYNC");
        broadcast_custom_data_deal((void *)wireless_trans->value);
        break;
 ...
 */
/* ----------------------------------------------------------------------------*/
static void app_broadcast_back_to_pair(void *p)
{
    bis_catch_timeout = 0;
    //重新进入配对模式
    app_broadcast_close(APP_BROADCAST_STATUS_STOP);
...
#if (BROADCAST_ENTER_PAIR_BEFORE_RUN && BIS_CATCH_TIMEOUT_ENABLE)
    if (!bis_catch_timeout) {
        bis_catch_timeout = sys_timeout_add(NULL, app_broadcast_back_to_pair, BIS_CATCH_TIME);
    }
#endif
...
{
    u8 *data = (u8 *)priv;
    g_printf("user_data[0]:0x%x", data[0]);
    g_printf("user_data[1]:0x%x", data[1]);
    return 0;
}
...
static void broadcast_pair_rx_event_callback(const PAIR_EVENT event, void *priv)
{
    ...
    case PAIR_EVENT_RX_PRI_CHANNEL_CREATE_SUCCESS:
        g_printf("PAIR_EVENT_RX_PRI_CHANNEL_CREATE_SUCCESS");
...
void app_broadcast_enter_pair(u8 mode)
{
    ...
    broadcast_enter_pair(BROADCAST_ROLE_RECEIVER, (void *)&pair_rx_cb, mode);
}
