...
#define MIC_RTN_CToP            4
#define IIS_RTN_CToP            3

#define AUX_RTN_PToC            3
#define MIC_RTN_PToC            4
...
static cig_parameter_t cig_mic_central_param = {
    ...
    .scan_int       = 16,
    .scan_win       = 5,
    .aclIntMs		= 60, //0: 12*iso_interval,unit:ms;
...
cig_parameter_t *set_cig_params(u8 app_task, u8 role, u8 pair_without_addr)
{
    ...
    if (role == CONNECTED_ROLE_CENTRAL) {
        ...
        ret = syscfg_read(VM_WIRELESS_PAIR_RX_BIND_CODE, &pair_bind_code, sizeof(pair_bind_code));
        if (ret <= 0) {
            pair_bind_code = 0;
        }

        central_params->pair_bind_code = pair_bind_code;
        g_printf("central pair_bind_code :\n");
        put_buf(&central_params->pair_bind_code, sizeof(central_params->pair_bind_code));
        central_params->pair_sw_time = 2000;
        ///主机 端 不绑定，始终保持公共配对码
        central_params->pair_public_code = SET_PRIVATE_ACCESS_ADDR;
        central_params->pair_rssi = (-120);
    ...
    if (role == CONNECTED_ROLE_PERIP) {
        ...
        perip_params->pair_bind_code = pair_bind_code;
        g_printf("perip pair_bind_code :\n");
        put_buf(&perip_params->pair_bind_code, sizeof(perip_params->pair_bind_code));
        perip_params->pair_sw_time = 2000;

        ///从机端 从机端要是有私有配对码的情况下就必须让公共配对码为0
        if(perip_params->pair_bind_code){
            perip_params->pair_public_code = 0;
        }else{
            perip_params->pair_public_code = SET_PRIVATE_ACCESS_ADDR;
        }

        perip_params->pair_rssi = (-120);
        return perip_params;
    }
