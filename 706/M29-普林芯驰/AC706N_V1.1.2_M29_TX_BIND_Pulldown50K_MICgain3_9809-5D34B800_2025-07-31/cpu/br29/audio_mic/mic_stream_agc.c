#include "mic_stream_agc.h"
#include "stream_entry.h"
#include "wireless_mic_effect.h"

#if WIRELESS_MIC_CONFIG_AGC

#define AGC_TASK_NAME   "mic_stream_agc"

#define AGC_STREAM_ASYNC_EN        0//1:异步处理, 会增加一个处理单元的延时， 0:同步处理， 不增加延时， 但是要求在一个cpu核要可以做完算法处理

static int adapter_stream_agc_data_handle(void *priv, struct audio_data_frame *in)
{
    /* putchar('$'); */
    struct __stream_agc *hdl = (struct __stream_agc *)priv;

    if (in->offset) {
        return in->data_len;
    }
#if (AGC_STREAM_ASYNC_EN)
    u8 ready = 1;
...
#else
    //agc处理算法
    audio_agc_run(hdl->agc_hdl, in->data, in->data, in->data_len);
#endif
...
static void adapter_stream_agc_task(void *priv)
{
    u32 r_len, w_len;
    struct __stream_agc *hdl = (struct __stream_agc *)priv;
    ASSERT(hdl);
        ...
        if (hdl->status) {
            ...
            if (r_len == hdl->frame_buf_size) {
                if (hdl->status) {
                    ...
                    //agc处理算法
                    audio_agc_run(hdl->agc_hdl, hdl->frame_buf, hdl->frame_buf, hdl->frame_buf_size);
...
{
    struct __stream_agc *hdl = zalloc(sizeof(struct __stream_agc));
    ASSERT(hdl);

    hdl->stream = stream_entry_open(hdl, adapter_stream_agc_data_handle, 0);
    ASSERT(hdl->stream);

    //打开agc
    hdl->agc_hdl = audio_agc_open(agc_param);
    ...
    ;
    int err = task_create(adapter_stream_agc_task, hdl, AGC_TASK_NAME);
...
void adapter_stream_agc_close(struct __stream_agc **p)
{
    ...
    if (p && hdl) {
        ...
        //关闭agc
        audio_agc_close(hdl->agc_hdl);

        task_kill(AGC_TASK_NAME);
        if (hdl->frame_buf) {
            free(hdl->frame_buf);
        }
        if (hdl->in_buf) {
