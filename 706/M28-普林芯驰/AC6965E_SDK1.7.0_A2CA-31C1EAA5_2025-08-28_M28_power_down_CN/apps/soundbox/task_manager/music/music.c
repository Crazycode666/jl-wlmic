...
*/
/*----------------------------------------------------------------------------*/
extern int bt_conn_flag;
extern int disk_flag;
extern void poweroff_dete();
extern int rec_turn_music;
...
void music_player_err_deal(int err)
{
    ...
    case MUSIC_PLAYER_ERR_DEV_OFFLINE:
        printf("MUSIC_PLAYER_ERR_DEV_OFFLINE \n");
...
static bool music_paly_repeat = true;
extern char rec_path[64];
bool line_mode_flag = 1;
static int music_key_event_opr(struct sys_event *event)
{
    ...
    case KEY_MUSIC_PLAYER_QUIT:
        ...
        //tone_play_by_path(tone_table[IDEX_TONE_USB_OUT], 1);
        break;
    ///结束消息处理
    ...
    case  KEY_MUSIC_PREV:
        ...
        //tone_play_by_path(tone_table[IDEX_TONE_PREV], 1);
    ...
    case  KEY_MUSIC_NEXT:
        ...
        //tone_play_by_path(tone_table[IDEX_TONE_NEXT], 1);
        app_status_handler(APP_STATUS_MUSIC_FFR);
        __this->file_play_direct = 0;
        err = music_player_play_next();
        break;
    case KEY_MUSIC_REPEAT:
        /*music_paly_repeat = !music_paly_repeat;
        if(music_paly_repeat)
            file_dec_repeat_set(10);
        else
            file_dec_repeat_set(1);*/
        //if(!line_mode_flag)
        /*{
            printf("*******************   rec_path = %s ********************\n",rec_path);
            music_player_play_by_path((char *)"udisk0", rec_path);///this is a demo
            record_file_play();
        }*/
    ...
    case SYS_DEVICE_EVENT:
        ...
        case DEVICE_EVENT_FROM_OTG:
            ...
            disk_flag = 0;
                ...
                if(line_mode_flag)
                    tone_play_by_path(tone_table[IDEX_TONE_USB_IN], 1);
                disk_flag = 1;
...
void app_music_task()
{
    ...
    gpio_set_output_value(IO_PORTB_06,1);
    int res;
    int msg[32];
    music_task_start();
    if(tone_music_flag && line_mode_flag)
        tone_play_by_path(tone_table[IDEX_TONE_MUSIC], 1);
    is_rec_mode = false;
#if (MUSIC_DEVICE_TONE_EN)
    music_player_play_start();
#else
    ...
    {
        //if(tone_music_flag)
            //tone_play_with_callback_by_name(tone_table[IDEX_TONE_NONE], 1, music_tone_play_end_callback, (void *)IDEX_TONE_NONE);
    }
#endif
