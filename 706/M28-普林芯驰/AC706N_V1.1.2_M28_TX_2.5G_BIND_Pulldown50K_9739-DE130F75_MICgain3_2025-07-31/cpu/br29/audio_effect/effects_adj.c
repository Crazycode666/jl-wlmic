...
int mic_gain_process_parm_analyze(EFF_ONLINE_PACKET *packet, u8 id, u8 sq)
{
#if (defined(TCFG_MIC_EFFECT_ENABLE) && TCFG_MIC_EFFECT_ENABLE) || (defined(WIRELESS_MIC_EFFECT_ENABLE) && WIRELESS_MIC_EFFECT_ENABLE)
#if defined(TCFG_MIC_GAIN_ENABLE) && TCFG_MIC_GAIN_ENABLE
    u8 mode_id = packet->data[0];
    Gain_Process_TOOL_SET *gain_parm = &eff_mode[mode_id - mic_mode_seq0].gain_parm;
    memcpy(gain_parm, &packet->data[1], sizeof(eff_mode[mode_id - mic_mode_seq0].gain_parm));
    audio_gain_update_parm(AEID_MIC_GAIN, &gain_parm->parm, gain_parm->is_bypass);
#if PARM_DEBUG

    log_debug("gain[0] 0x%x, gain[1] 0x%x, gain[2] 0x%x, gain[3] 0x%x, bypass %d\n",
              *(int *)&gain_parm->parm.gain[0], *(int *)&gain_parm->parm.gain[1], *(int *)&gain_parm->parm.gain[2], *(int *)&gain_parm->parm.gain[3], gain_parm->is_bypass);
#endif
#endif
#endif
    return 0;
...
#if TCFG_EFFECT_TOOL_ENABLE
    .cmd = EFF_MIC_GAIN,
    .cmd_deal = mic_gain_process_parm_analyze,
#endif
