...
static u32 alink_isr_get_len(void *hw_alink, u8 ch)
{
    ALINK_PARM *hw_alink_parm = (ALINK_PARM *)hw_alink;
...
static void alink_sr(void *hw_alink, u32 rate)
{
    ...
    u32 pll_target_frequency = clk_get("pll");;
    ...
    case ALINK_SR_44100:
        ...
        mclk_fre = 11289600;
        ALINK_MDIV(module, MCLK_DIV_1);
        if (pll_target_frequency == 192 * MHz || pll_target_frequency == 288 * MHz) {
            ALINK_LRDIV(module, MCLK_LRDIV_256FS);
        } else if (pll_target_frequency == 240 * MHz) {
...
void *alink_channel_init(void *hw_alink, ALINK_CH ch_idx, u8 dir, void *priv, void (*handle)(void *priv, void *addr, int len))
{
    ...
    alink_printf(">>> alink_channel_init %x\n", hw_alink_parm->ch_cfg[ch_idx].buf);
    u32 *buf_addr;
    ...
    *buf_addr = (u32)(hw_alink_parm->ch_cfg[ch_idx].buf);
    ...
    if (hw_alink_parm->ch_cfg[ch_idx].dir == ALINK_DIR_RX) {
        alink_io_in_init(alink0_IOS_port[IIS_IO_CH0 + ch_idx]);
    } else {
        alink_io_out_init(alink0_IOS_port[IIS_IO_CH0 + ch_idx]);
    }
    ...
    alink_printf("alink%d, ch%d init\n", module, ch_idx);
...
static void alink0_info_dump()
{
    ...
    alink_printf("JL_CLOCK->PRP_CON1 = 0x%x", JL_CLOCK->PRP_CON1);
...
void *alink_init(void *hw_alink)
{
    ...
    /* ALINK_MSRC(module, MCLK_EXTERNAL);	 */
    ALINK_MSRC(module, MCLK_PLL_CLK);

    if (module == ALINK0_PORTA) {
        for (int i = 0; i < 7; i++) {
            alink0_IOS_port[i] = alink0_IOS_portA[i];
        }
        JL_IOMC->IOMC0 &= ~BIT(15);
        alink_printf("alink0_portA init\n");
    } else if (module == ALINK0_PORTB) {
        for (int i = 0; i < 7; i++) {
            alink0_IOS_port[i] = alink0_IOS_portB[i];
        }
        JL_IOMC->IOMC0 |= BIT(15);
        alink_printf("alink0_portB init\n");
    }
    ...
    if (hw_alink_parm->role == ALINK_ROLE_MASTER) {
        //主机输出时钟
        alink_io_out_init(alink0_IOS_port[IIS_IO_MCLK]);
        alink_io_out_init(alink0_IOS_port[IIS_IO_SCLK]);
        alink_io_out_init(alink0_IOS_port[IIS_IO_LRCK]);
        ALINK_MOE(module, 1);				/*MCLK output to IO*/
        ALINK_SOE(module, 1);				/*SCLK/LRCK output to IO*/
    } else {
        //从机输入时钟
        alink_io_in_init(alink0_IOS_port[IIS_IO_MCLK]);
        alink_io_in_init(alink0_IOS_port[IIS_IO_SCLK]);
        alink_io_in_init(alink0_IOS_port[IIS_IO_LRCK]);
...
void alink_channel_close(void *hw_channel)
{
    ...
    gpio_set_pull_up(alink0_IOS_port[IIS_IO_CH0 + hw_channel_parm->ch_idx], 0);
    gpio_set_pull_down(alink0_IOS_port[IIS_IO_CH0 + hw_channel_parm->ch_idx], 0);
    gpio_set_direction(alink0_IOS_port[IIS_IO_CH0 + hw_channel_parm->ch_idx], 1);
    gpio_set_die(alink0_IOS_port[IIS_IO_CH0 + hw_channel_parm->ch_idx], 0);
...
void audio_link_test(void)
{
    ...
    alink_ch0 =  alink_channel_init(hw_alink, 0, ALINK_DIR_TX, NULL, handle_tx);
    /* alink_ch1 =  alink_channel_init(hw_alink, 1, ALINK_DIR_RX, NULL, handle_rx); */
    /* alink_ch2 =  alink_channel_init(hw_alink, 2, ALINK_DIR_TX, NULL, handle_tx); */
    /* alink_ch3 =  alink_channel_init(hw_alink, 3, ALINK_DIR_RX, NULL, handle_rx); */
