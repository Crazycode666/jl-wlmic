 ...
 * @brief 获取对应链路的rssi
 *
 * @param cis_hdl:指定的链路句柄
 *
 * @return 信号强度
 */
/* ----------------------------------------------------------------------------*/
int connected_get_rssi(u16 cis_hdl)
{
    int rssi = wireless_dev_get_rssi("cig_rx", cis_hdl);
    return rssi;
}
            ...
            if (connected_hdl->cis_hdl_info[i].cis_hdl) {
                ...
                if (connected_hdl->capture) {
                    if (!capture_send_update) {
                        ...
                        live_audio_capture_send_update(connected_hdl->capture, get_cig_sdu_period_us(), timestamp);
                        capture_send_update = 1;
                    }
                } else {
                    live_audio_capture_send_update(cis_hdl_info->capture, get_cig_sdu_period_us(), timestamp);
                }
            }
        }
...
static void connected_central_event_callback(const CIG_EVENT event, void *priv)
{
    ...
    case CIG_EVENT_ACL_CONNECT:
        ...
        break;
        case CIG_EVENT_ACL_RX_BIND_CODE:
        printf("%s, CIG_EVENT_ACL_RX_BIND_CODE\n", __FUNCTION__);
       uint32_t  u_pair_bind_code = *((uint32_t *)priv);
       put_buf(&u_pair_bind_code, sizeof(uint32_t)); 
       u32 pair_bind_code = 0;

       syscfg_read(VM_WIRELESS_PAIR_RX_BIND_CODE, &pair_bind_code, sizeof(pair_bind_code));
       if(u_pair_bind_code != pair_bind_code){
           int ret = syscfg_write(VM_WIRELESS_PAIR_RX_BIND_CODE, &u_pair_bind_code, sizeof(u_pair_bind_code));
           if (ret <= 0) {
               r_printf(">>>>>>VM_WIRELESS_PAIR_RX_BIND_CODE save err ret:%d",  ret);
           }
       }
       break;
