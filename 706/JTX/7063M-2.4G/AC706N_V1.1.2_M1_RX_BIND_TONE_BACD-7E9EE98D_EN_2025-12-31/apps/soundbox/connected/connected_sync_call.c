...
static int connected_data_rx_suss(u16 acl_hdl, void *data, int len)
{
    ...
    //printf("####################1111111111111111111  DATA = %d ###################\n",data);
    //int u_buffer[2] = {0};
    //memcpy(u_buffer,(u8 *)data,2);
...
static void data_transmit_handler(u16 acl_hdl, u8 arg_num, int *argv)
{
    ...
    u8 *data = (u8 *)argv[1];
    int len = argv[2];
    //printf("%s %d, length:%d, trans_role:%d", __FUNCTION__, __LINE__, len, trans_role);
    //put_buf(data, len);
    ...
    if(data[0] == 161)
    {
        if(data[1])
        {
            //tone_play_index(IDEX_TONE_MIC_ON, 0);
           tone_play_by_path(TONE_MIC_OFF, 1);
        }
        else{
            //tone_play_index(IDEX_TONE_MIC_OFF, 0);
            tone_play_by_path(TONE_MIC_ON, 1);
        }
    }
    if(data[0] == 160)
    {
        if(data[1])
            tone_play_by_path(TONE_DU, 1);
    }
...
CONN_DATA_TRANS_STUB_REGISTER(data_transmit) = {
    .uuid = CONNECTED_DATA_SYNC_FUNC_ID,
