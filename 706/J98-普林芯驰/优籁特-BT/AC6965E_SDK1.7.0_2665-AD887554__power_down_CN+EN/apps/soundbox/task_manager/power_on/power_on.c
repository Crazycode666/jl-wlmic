...
{
    printf("################  BT FLAG ################# %d\n",bt_conn_flag);
    if(connect_7063_flag || bt_conn_flag || disk_flag)
    {
        read_state_flag1 = 0;
...
void app_poweron_task()
{
    ...
    if(language_mode_flag)
        ...
        int err =  tone_play_with_callback_by_name(tone_en_table[IDEX_TONE_POWER_ON], 1, tone_play_end_callback, (void *)IDEX_TONE_POWER_ON);
