...
int bt_key_event_handler(struct sys_event *event)
{
    ...
    case  KEY_MUSIC_PP:
        ...
        /*if(music_state)
        {
            tone_play_by_path(tone_table[IDEX_TONE_PLAY], 1);
        }
        else
            tone_play_by_path(tone_table[IDEX_TONE_PAUSE], 1);*/
        break;
    case  KEY_MUSIC_PREV:
    ...
    case KEY_CHANGE_MODE:
        //tone_play_by_path(tone_table[IDEX_TONE_REPEAT], 1);
        tone_play_with_callback_by_name(tone_table[IDEX_TONE_MUSIC], 1, NULL, NULL);
        bt_conn_flag = 0;
        app_task_switch_to(APP_MUSIC_TASK);


        break;
    case KEY_LANAGUAGE_MODE:
...
void app_bt_task()
{
    ...
    gpio_direction_input(IO_PORTA_00);
    ...
    if (!__this->cmd_flag && (!get_tws_background_connected_flag())) { //蓝牙后台拉回蓝牙模式不播放提示音
        if(language_mode_flag)
            ...
            tone_play_with_callback_by_name(tone_en_table[IDEX_TONE_BT_MODE], 1, bt_tone_play_end_callback, (void *)IDEX_TONE_BT_MODE);
        //协议栈初始化在提示音结束进行
    } else {
