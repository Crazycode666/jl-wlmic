...
int music_player_change_repeat_mode(void)
{
    if (__this) {
        ...
        if(app_var.cycle_mode ==1)
        {
            if(language_mode_flag)
                tone_play_by_path(tone_table[IDEX_TONE_LOOP], 1);
            else
                tone_play_by_path(tone_en_table[IDEX_TONE_LOOP], 1);
        }
        else if(app_var.cycle_mode == 2)
        {
            if(language_mode_flag)
                tone_play_by_path(tone_table[IDEX_TONE_REPEAT], 1);
            else
                tone_play_by_path(tone_en_table[IDEX_TONE_REPEAT], 1);
        }
        else if(app_var.cycle_mode == 3)
        {
            if(language_mode_flag)
                tone_play_by_path(tone_table[IDEX_TONE_RAND], 1);
            else
                tone_play_by_path(tone_en_table[IDEX_TONE_RAND], 1);
        }
