...
extern void echo_led_on();
extern void echo_led_off(u32 gpio_port,u8 sw);
...
void app_connected_conn_status_event_handler(struct wireless_trans_event *wireless_trans)
{
    ...
    case CIG_EVENT_CENTRAL_CONNECT:
        ...
        if(cis_connected_nums == 0)
        {
            ...
            //ui_user_mode_set(STATUS_BT_CONN, PWM_LED1_OFF, 0);
            led_flash_del();
            echo_led_on(IO_PORTB_05,1);
        }
        else if(cis_connected_nums == 1)
        {
            led_flash_del();
            echo_led_off(IO_PORTB_05,1);
            gpio_set_direction(IO_PORTB_01,0);
            gpio_direction_output(IO_PORTB_01,1);
            //ui_user_mode_set(STATUS_BT_CONN, PWM_LED1_ON, 0);
            echo_led_on(IO_PORTA_01,0);
        }
        else{
            gpio_set_direction(IO_PORTB_01,0);
            gpio_direction_output(IO_PORTB_01,1);
    ...
    case CIG_EVENT_CENTRAL_DISCONNECT:
        ...
        else if(cis_connected_nums == 0)
        {
            led_flash_add();
            //ui_user_mode_set(STATUS_BT_DISCONN, PWM_LED1_FAST_FLASH, 0);
            gpio_set_direction(IO_PORTB_01,0);
 ...
 * @brief 获取对应设备的rssi
 *
 * @param device_channel:指定的设备
 *
 * @return 信号强度
 */
/* ----------------------------------------------------------------------------*/
int app_connected_get_rssi(u8 device_channel)
{
    u8 i, j;
    int rssi;
    struct connected_sync_channel *p;
        ...
        if (app_cig_conn_info[i].used) {
            for (j = 0; j < LEA_CIG_CONNECTION_NUM; j++) {
                if (app_cig_conn_info[i].cis_conn_info[j].remote_dev_identification & device_channel) {
                    rssi = connected_get_rssi(app_cig_conn_info[i].cis_conn_info[j].cis_hdl);
                }
            }
        }
    }
