...
{
    u8 pinr_io, reg;
    reg = P33_CON_GET(P3_PINR_CON);
    //开启LDO5V_DET长按复位
    if ((reg & BIT(0)) && (reg & BIT(6)) && ((reg & BIT(7)) == 0)) {
        if (level == 0) {
...
void set_charge_mA(u16 charge_mA)
{
    ...
    for (vref = 0; vref < 4; vref++) {
        for (chgi = 0; chgi < 1024; chgi++) {
            if (type) {
                temp = (float)(1 + 0.05 * (vref + 1)) * 0.8 / 20.48 * (chgi + 1);
            } else {
                temp = (float)(1 + 0.05 * (vref + 1)) * 0.8 / 2.048 * (chgi + 1);
            }
...
_check_end:
    if (charge_mA >= 30) {
        ...
        charge_curr_trim = get_charge_cur_trim();
    ...
    CHGI_TRIM_SEL(charge_curr_trim);//电流校准
