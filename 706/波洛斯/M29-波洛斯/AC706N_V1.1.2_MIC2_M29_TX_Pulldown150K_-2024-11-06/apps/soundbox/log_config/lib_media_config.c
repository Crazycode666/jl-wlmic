...
//vcm通路(cap/capless)切换时间,单位(ms)
const int config_audio_dac_vcm_chsel_delay = 800;
...
/* 置1后延时会降低400us左右，BROADCAST_SOURCE_IRQ_SYNC_TX_ALIGN设0时会使扫频曲线高频部分会变得不平直。
如果BROADCAST_SOURCE_IRQ_SYNC_TX_ALIGN为0且对扫频有要求，可把变量置0。
如果BROADCAST_SOURCE_IRQ_SYNC_TX_ALIGN不为0且对扫频有要求, 只需改RX端该变量即可 */
const int config_src_low_latency = 1;
...
#if (SOUNDCARD_ENABLE || TCFG_MIC_EFFECT_ENABLE || RECORDER_MIX_EN ||TCFG_DYNAMIC_EQ_ENABLE)
...
const int JLA_PLC_EN = 0;   //0:淡入淡出; 1:时域PLC;  2:频域PLC;  3:无任何效果，仅补静音包;
...
#if JL_CC_CODED_EN
const int JLA_CODEC_HARD_DECISION_ENABLE = 1;
#else
...
//*   	Reverb          *
//***********************
const int plate_reverb_mode = REVERB_ORINGIN_MODE; //reverb模式选择
...
const char log_tag_const_i_AUDIO_ENCODER AT(.LOG_TAG_CONST) = CONFIG_DEBUG_LIB(1);
