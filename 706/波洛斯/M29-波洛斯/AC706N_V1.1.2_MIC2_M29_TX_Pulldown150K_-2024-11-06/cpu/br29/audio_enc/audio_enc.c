...
void audio_adc_capless_trim()
{
    u8 ret = 0;
    struct capless_trim trim_info;
#if (TCFG_MC_BIAS_AUTO_ADJUST == MC_BIAS_ADJUST_ALWAYS)
    ret = audio_mic_bias_adjust(&trim_info);
    if (ret == 0) {
        syscfg_write(CFG_MC_BIAS, &trim_info, sizeof(struct capless_trim));
        adc_hdl.pd->mic_bias_res = trim_info.bias_rsel;
        puts("mic_capless_trim_success");
    } else {
        puts("mic_capless_trim_error");
    }
#elif (TCFG_MC_BIAS_AUTO_ADJUST == MC_BIAS_ADJUST_ONE)
    u8 capless_trim_result = 0;
    int len = syscfg_read(CFG_MC_BIAS, &trim_info, sizeof(struct capless_trim));
    printf("MIC_CAPLESS_TRIM:%d\n", trim_info.bias_rsel);
    if (len != sizeof(struct capless_trim)) {
        ret = audio_mic_bias_adjust(&trim_info);
        if (ret == 0) {
            syscfg_write(CFG_MC_BIAS, &trim_info, sizeof(struct capless_trim));
            adc_hdl.pd->mic_bias_res = trim_info.bias_rsel;
...
AT(.audio_adc_irq.text.cache.L2)
...
static int audio_adc_mode_digital_enable(u8 enable)
{
    if (adc_mode_hdl == NULL) {
        printf("%s %d hdl is NULL err !\n", __func__, __LINE__);
        return -1;
    ...
    audio_adc_mic_digital_enable(&adc_mode_hdl->adc_ch, enable);
    return 0;
}
...
#ifndef CONFIG_CPU_BR29)  //br29只有一个adc 通道不需要申请buf做多通道数据分离
...
void audio_mic_digital_enable(u8 enable)
{
    audio_adc_mode_digital_enable(enable);
}
...
#if (TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_1)
        audio_mic_pwr_io(TCFG_AUDIO_MIC1_PWR_PORT, 0);//MIC1
#endif/*TCFG_AUDIO_ADC_MIC_CHA*/
