...
static void spi0_iomc_config(spi_dev spi)
{
    ...
    JL_IOMC->IOMC0 &= ~ BIT(16);
    JL_IOMC->IOMC0 &= ~ BIT(17);
    if (port == 'A') {
        if (spi_io_map[id].io[port - 'A'].do_pin != IO_PORTD_01) {
            do_sel = 1;
        }
    } else if (port == 'B') {
        JL_IOMC->IOMC0 |= BIT(17);
        if (spi_io_map[id].io[port - 'A'].do_pin != IO_PORTB_07) {
            do_sel = 1;
        }
    }
    if (do_sel) {
        JL_IOMC->IOMC0 |= BIT(16);
        gpio_och_sel_output_signal(spi_io_map[id].io[port - 'A'].do_pin, OUTPUT_CH_SIGNAL_SPI0_DO);
    }
//spi0 no 4bit
}
...
static void spi1_iomc_config(spi_dev spi)
{
    ...
    u8 id = spi_get_info_id(spi);
    u8 port = spi_get_info_port(spi);
    JL_IOMC->IOMC0 &= ~(BIT(18) | BIT(19) | BIT(20));
    if (port == 'A') {
        i = 0;
        if (spi_io_map[id].io[port - 'A'].do_pin != IO_PORTA_03) {
            do_sel = 1;
        }
    } else if (port == 'B') {
        i = 1;
        if (spi_io_map[id].io[port - 'A'].do_pin != IO_PORTC_05) {
            do_sel = 1;
        }
    } else if (port == 'C') {
        i = 2;
        ...
        i = 3;
        if (spi_io_map[id].io[port - 'A'].do_pin != IO_PORT_DM) {
            do_sel = 1;
        }
    }
    if (do_sel) {
        JL_IOMC->IOMC0 |= BIT(18);
        gpio_och_sel_output_signal(spi_io_map[id].io[port - 'A'].do_pin, OUTPUT_CH_SIGNAL_SPI1_DO);
    }

    SFR(JL_IOMC->IOMC0, 19, 2, i);
}
...
void spi_set_bit_mode(spi_dev spi, int mode)
{
    ...
    case SPI_MODE_UNIDIR_4BIT:
        ASSERT(id == 1, "spi%d not support SPI_MODE_UNIDIR_4BIT\n", id);
        spi_unidir(spi_regs[id]);
    ...
    if (mode == SPI_MODE_BIDIR_1BIT) {
        ...
        spi_io_port_init(spi_io_map[id].io[port].do_pin, role == SPI_ROLE_MASTER ? 0 : 1);
        spi_io_port_init(spi_io_map[id].io[port].di_pin, role == SPI_ROLE_MASTER ? 0 : 1);
    } else if (mode == SPI_MODE_UNIDIR_4BIT) {
        spi_io_port_init(spi_io_map[id].io[port].do_pin, role == SPI_ROLE_MASTER ? 0 : 1);
        spi_io_port_init(spi_io_map[id].io[port].di_pin, role == SPI_ROLE_MASTER ? 0 : 1);
        spi_io_port_init(spi_io_map[id].io[port].d2_pin, role == SPI_ROLE_MASTER ? 0 : 1);
        spi_io_port_init(spi_io_map[id].io[port].d3_pin, role == SPI_ROLE_MASTER ? 0 : 1);
    }
}
...
int spi_open(spi_dev spi)
{
    ...
    printf("spi%d IOMC1(16~17,18~20)   = %08x\n", id, JL_IOMC->IOMC0);
#endif
    return 0;
}
