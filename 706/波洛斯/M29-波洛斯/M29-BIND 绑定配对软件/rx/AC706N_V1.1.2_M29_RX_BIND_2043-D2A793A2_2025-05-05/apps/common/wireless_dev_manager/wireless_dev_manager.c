 ...
 */
/* ----------------------------------------------------------------------------*/
int wireless_dev_enter_pair(const char *name, u8 mode, void *priv)
{
    int ret = -1;
    const wireless_dev_ops *wireless_dev;
    WIRELESS_DEV_ENTER_CRITICAL();
    list_for_each_wireless_dev(wireless_dev) {
        if (!strcmp(name, wireless_dev->name)) {
            if (wireless_dev->ioctrl) {
                ret = wireless_dev->ioctrl(WIRELESS_DEV_OP_ENTER_PAIR, mode, priv);
            }
            WIRELESS_DEV_EXIT_CRITICAL();
            return ret;
        }
 ...
 */
/* ----------------------------------------------------------------------------*/
int wireless_dev_get_rssi(const char *name, u16 hdl)
{
    int rssi = 0;
    const wireless_dev_ops *wireless_dev;
    WIRELESS_DEV_ENTER_CRITICAL();
    list_for_each_wireless_dev(wireless_dev) {
        if (!strcmp(name, wireless_dev->name)) {
            if (wireless_dev->ioctrl) {
                rssi = wireless_dev->ioctrl(WIRELESS_DEV_OP_GET_RSSI, hdl);
            }
            WIRELESS_DEV_EXIT_CRITICAL();
            return rssi;
        }
