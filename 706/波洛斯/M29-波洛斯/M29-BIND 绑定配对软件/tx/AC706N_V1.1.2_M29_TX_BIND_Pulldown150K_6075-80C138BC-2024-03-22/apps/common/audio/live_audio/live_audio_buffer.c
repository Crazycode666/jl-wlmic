...
#define __AUDIO_BUFFER_RUN AT(.audio_buffer.text.cache.L2)
...
int live_audio_buffer_read(void *audio_buffer, void *data, int len)
{
    ...
    spin_lock(&audio_buf->lock);
    list_for_each_entry_safe(frame, n, &audio_buf->frame_head, entry) {
        u32 rlen = frame->frame.len - frame->frame.offset;
        if (rlen > len) {
            rlen = len;
        }
        ASSERT((offset + rlen) <= 2048, "data:0x%x, offset:0x%x, rlen:%d", data, offset, rlen);
