...
//dec+sync latency
#define LATENCY_IO_DEBUG_1(i,x)       //{JL_PORT##i->DIR &= ~BIT(x), JL_PORT##i->OUT |= BIT(x);}
#define LATENCY_IO_DEBUG_0(i,x)       //{JL_PORT##i->DIR &= ~BIT(x), JL_PORT##i->OUT &= ~BIT(x);}
...
#elif (WIRELESS_MIC_EFFECT_ENABLE && WIRELESS_MIC_RX_EFFECT_ENABLE)
...
#if (TCFG_CONNECTED_ENABLE && (LEA_CIG_CONNECTION_NUM == 2))
#define LIVE_STREAM_PALYER_USE_ULTRA_LOW_LATENCY  2000//1000
#else
#define LIVE_STREAM_PALYER_USE_ULTRA_LOW_LATENCY  0
#endif
#endif

#if (WIERLESS_TRANS_CODING_TYPE == LIVE_AUDIO_CODING_JLA_LL)
#if (TCFG_CONNECTED_ENABLE && (LEA_CIG_CONNECTION_NUM == 2))
#define LIVE_STREAM_PALYER_USE_ULTRA_LOW_LATENCY  2000
#else
#define LIVE_STREAM_PALYER_USE_ULTRA_LOW_LATENCY  900
#endif
#endif

/*下行播放延时设置*/
#if JL_CC_CODED_EN
#define LIVE_STREAM_RX_LATENCY                  (800L)
#else
#define LIVE_STREAM_RX_LATENCY                  (300L)
#endif

#if (TCFG_BROADCAST_MODE_EQ_ENABLE || TCFG_BROADCAST_MODE_DRC_ENABLE)
#define LIVE_STREAM_EFFECT_LATENCY              (3 * 1000L)
#else
#define LIVE_STREAM_EFFECT_LATENCY              (1 * 500L)
#endif /*(TCFG_BROADCAST_MODE_EQ_ENABLE || TCFG_BROADCAST_MODE_DRC_ENABLE)*/

#define LIVE_JLA_STREAM_DECODING_LATENCY(frame_len, nch)    \
    (((frame_len > 50 ? 2 : 1) * 500L) + (nch * 500L / 2))
...
static int live_player_effect_data_handler(struct audio_stream_entry *entry,  struct audio_data_frame *in, struct audio_data_frame *out)
{
    ...
    if (effect->syncts) {
        ...
        int frame_len = 0;
        int offset = 0;
        int max_frame_len = effect->sample_rate * 2 / 1000;
        while (offset < in->data_len) {
            frame_len = (in->data_len - offset) > max_frame_len ? max_frame_len : (in->data_len - offset);
            wlen = audio_syncts_frame_filter(effect->syncts, (u8 *)in->data + offset, frame_len);
            offset += wlen;
            if (wlen < frame_len) {
                audio_syncts_trigger_resume(effect->syncts, (void *)entry, (void (*)(void *))audio_stream_resume);

                //audio_syncts_push_data_out(effect->syncts);//这里的使用可以避免极小概率发生的一直UUUuu的情况
                audio_syncts_try_latch_audio(effect->syncts);
                return offset;
            }
        ...
        return offset;
#else
        wlen = audio_syncts_frame_filter(effect->syncts, in->data, in->data_len);
        if (wlen < in->data_len) {
            audio_syncts_trigger_resume(effect->syncts, (void *)entry, (void (*)(void *))audio_stream_resume);
        }
        return wlen;
...
static int live_stream_player_start(struct live_stream_player_context *ctx)
{
    ...
    clock_add_set(LIVE_STREAM_DEC_CLK);
...
static int live_stream_player_stop(struct live_stream_player_context *ctx)
{
    ...
    clock_remove_set(LIVE_STREAM_DEC_CLK);
...
#if !LIVE_STREAM_PALYER_USE_ULTRA_LOW_LATENCY
...
#else
        ctx->play_latency = LIVE_STREAM_PALYER_USE_ULTRA_LOW_LATENCY;
#endif
        break;
#endif
