...
#endif
/* struct hw_iic_config hw_iic_cfg[] = { */
/*     //iic0 data */
/*     { */
/*         .port = 'A', // */
/*         .baudrate = 400000,      //IIC通讯波特率 */
/*         .hdrive = 0,             //是否打开IO口强驱 */
/*         .io_filter = 2,          //是否打开滤波器（去纹波）//0:close; 1:<1*Tiic_baud_clk, 2:<2*Tiic_baud_clk, 3:<3*Tiic_baud_clk */
...
int hw_iic_set_baud(hw_iic_dev iic, u32 baud)
{
    ...
    if (baud_cnt > 3) {
        iic_tsu_reg(iic_regs[id]) = 2; //<=127
        iic_thd_reg(iic_regs[id]) = 2; //<=127
    }
...
int hw_iic_init(hw_iic_dev iic)
{
    ...
    if (iic_info_role(iic) == IIC_MASTER) {
        ...
        iic_tsu_reg(iic_regs[id]) = 3; //<=127 sda信号保持时间
        iic_thd_reg(iic_regs[id]) = 3; //<=127 sda信号建立时间
    }
    ...
    if (iic_info_ie_en(iic)) {
        request_irq(IRQ_IIC_IDX, 3, (u32)hw_iic_slave_isr, 0);//3: 中断优先级
...
u8 hw_iic_start_addr(hw_iic_dev iic, u8 addr)
{
    ...
    if (addr_cnt == 1) {
        ...
        /* log_info("start IIC_PND 0x%x", iic_regs[id]->PND); */
...
void hw_iic_slave_isr()//slave  no_stretch
{
    ...
    if (hw_iic_get_pnd(iic, I2C_PND_TXTASK_LOAD)) {
        ...
        /* hw_i2c_put_task(iic, I2C_TASK_SEND_DATA); */
        mode_cnt = 2;
        SFR(iic_png_reg(iic_regs[id]), 10, 9, 0x1fb); //clr all png
        return;
    }
