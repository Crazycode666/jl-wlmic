...
#if (TCFG_USER_BLE_ENABLE || LEA_BIG_CTRLER_TX_EN || LEA_BIG_CTRLER_RX_EN || LEA_CIG_CENTRAL_EN || LEA_CIG_PERIPHERAL_EN || LE_AUDIO_TEST_EN)
        | BT_MODULE_LE
#endif//TCFG_USER_BLE_ENABLE
        ;
...
const uint64_t config_btctler_le_features = LE_FEATURES_BIS | LE_CORE_V50_FEATURES | LE_EXTENDED_ADVERTISING | LE_PERIODIC_ADVERTISING;
...
const int config_btctler_le_iso_tx = 0;
#if JL_CC_CODED_EN
const int config_btctler_le_iso_rx = 4096;
#else
const int config_btctler_le_iso_rx = 1024;
#endif

const int config_btctler_le_master_multilink = 0;
const int config_btctler_le_afh_en           = 0;

const u32 config_vendor_le_bb = 0; //VENDOR_BB_PIS_EN | VENDOR_BB_PIS_HB | VENDOR_BB_VENDOR_AFH;
...
#if (JL_CC_CODED_EN || JL_MPR_HARD_CMB_EN)
    (BIT(0)) |
#endif
#if JL_MPR_HARD_CMB_EN
    (BIT(4)) |
#endif
#if JL_CC_CODED_EN
    (BIT(1) | BIT(2)) |
...
const int config_btctler_le_roles =
#if LEA_CIG_CENTRAL_EN  //le_adv | le slave for update
    (LE_SCAN | LE_INIT | LE_MASTER | LE_ADV | LE_SLAVE) |
...
const uint64_t config_btctler_le_features = LE_FEATURES_CIS | LE_CORE_V50_FEATURES | LL_FEAT_ISO_HOST_SUPPORT;
...
const int config_btctler_le_iso_tx = 0;
const int config_btctler_le_iso_rx = 2000;//1280;
...
//BIT(15): S_CLS_SEVT
//BIT(16): SEN_OR_INT
//BIT(17): Optimized Code
//BIT(18): RSSI_GET_EN
const int config_bb_optimized_ctrl = BIT(13) | BIT(14) | BIT(16) | BIT(17);// | BIT(18);

#elif (LE_AUDIO_TEST_EN)

const int config_btctler_le_roles =
#if (LE_AUDIO_TEST_EN & BIT(0))
    (LE_ADV | LE_SLAVE) |
#elif (LE_AUDIO_TEST_EN & BIT(1))
    (LE_SCAN | LE_MASTER) |
#endif
    0;
