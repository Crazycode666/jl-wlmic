...
const char *bt_get_ble_name()
{
    return (const char *)(bt_cfg.ble_name);
}

const char *bt_get_product_test_name()
{
    return PRODUCT_TEST_NAME;
}

const char *ble_update_testbox_name()
{
    static char testbox_update_name[LOCAL_NAME_LEN + 4];
    snprintf(testbox_update_name, strlen(bt_cfg.ble_name) + 4, "%s_tx", bt_cfg.ble_name);
    return testbox_update_name;
}
