...
CHARGE_PLATFORM_DATA_BEGIN(charge_data)
    ...
    .charge_trickle_mA		= TCFG_CHARGE_TRICKLE_MA|BIT(9),          //涓流充电电流
/*ldo5v拔出过滤值，过滤时间 = (filter*2 + 20)ms,ldoin<0.6V且时间大于过滤时间才认为拔出
 ...
 充电仓，需要根据实际情况设置该值大小*/
    ...
    .ldo5v_pulldown_lvl     = CHARGE_PULLDOWN_150K,            //下拉电阻档位选择
...
#ifndef TCFG_AUDIO_MIC_BIAS_EN
#define TCFG_AUDIO_MIC_BIAS_EN             0
#endif/*TCFG_AUDIO_MIC_BIAS_EN*/
...
/*MIC免电容方案需要设置，影响MIC的偏置电压
    ...
    .mic_bias_res   = 4,
	.mic_bias_inside = TCFG_AUDIO_MIC_BIAS_EN,
...
const struct iokey_port iokey_list[] = {
...

    {
        ...
        .key_type.two_io.out_port = TCFG_IOKEY_MODE_OUT_PORT,
        .key_type.two_io.in_port = TCFG_IOKEY_MODE_IN_PORT,
        .key_value = 3,
    },*/
};
...
/************************** IIS config ****************************/
#define ALINK_IPERIOD(PERIOD) ((TCFG_IIS_SR / 1000 + 15) / 16 * 16 * PERIOD / 1000)

#if (TCFG_AUDIO_INPUT_IIS || TCFG_AUDIO_OUTPUT_IIS)
ALINK_PARM alink0_platform_data = {
    .module = TCFG_IIS_PORT,
    .mode = ALINK_MD_IIS,
#if TCFG_IIS_MODE
    ...
    .buf_mode = ALINK_BUF_CIRCLE, //dma_len = ALNK_BUF_POINTS_NUM * 2 * 2
#ifdef TCFG_IIS_CAPTURE_SAMPLE_PERIOD
    .iperiod = ALINK_IPERIOD(TCFG_IIS_CAPTURE_SAMPLE_PERIOD),
    .dma_len = ALINK_IPERIOD(TCFG_IIS_CAPTURE_SAMPLE_PERIOD) * 3 * 2 * 2,
#else
    ...
    /* .buf_mode = ALINK_BUF_DUAL,//dma_len = ALNK_BUF_POINTS_NUM * 2 * 2 * 2 */
};
#endif
/************************** PWM_LED ****************************/
...
u8 get_power_on_status(void)
{
    ...
    if (adkey_data.enable) {
        if (adc_get_value_blocking(adkey_data.ad_channel) < 10) {
            return 1;
        }
    }
#endif
...
#endif
	{
    	power_set_current_mode(PWR_LDO15);
	}else{
    	power_set_current_mode(TCFG_LOWPOWER_POWER_SEL);
	}
...
void board_set_soft_poweroff(void)
{
    ...
    soff_gpio_protect(TCFG_CHIP_RESET_PIN);
#endif
...
#endif /* #ifdef CONFIG_BOARD_AC706N_2T1_SIMPLEX */
