...
void *sound_iis_capture_open(struct audio_path *path)
{
    ...
    iis->sample_rate = TCFG_IIS_SR;//path->fmt.sample_rate;
    ...
    //默认adc中断点数为ALNK_BUF_POINTS_NUM
    u16 iis_irq_points = ALNK_BUF_POINTS_NUM;
    //需要声道转换时才申请转换缓存
    if (iis->nch < 2) {
        if (alink0_platform_data.iperiod) {
            iis_irq_points = alink0_platform_data.iperiod;
        }
        iis->sample_buffer = zalloc(iis_irq_points * 2 * iis->nch);
        ASSERT(iis->sample_buffer);
    }
