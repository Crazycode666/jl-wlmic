...
struct hw_eq_ch {
    char out_32bit;
    char updata;
    char updata_coeff_only;	// 只更新参数，不更新中间数据
    char no_wait;
    char channels;
    char SHI;
    char countL;
...
void butterworth_hp_design(int fc, int fs, int nSOS, int *coeff);
void  butterworth_lp_design(int fc, int fs, int nSOS, int *coeff);
