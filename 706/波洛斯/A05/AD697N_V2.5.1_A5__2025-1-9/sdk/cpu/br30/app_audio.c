...
#if (TCFG_CALL_USE_DIGITAL_VOLUME || (SYS_VOL_TYPE == VOL_TYPE_DIGITAL))
    if (__this->state == APP_AUDIO_STATE_CALL) {
...
void audio_gain_dump()
{
    ...
    u8 mic0_6dB_en = (JL_ADDA->ADA_CON1 & 0x1);
    u8 mic1_gain = (JL_ADDA->ADA_CON0 >> 5) & 0x1F;
    u8 mic1_6dB_en = (JL_ADDA->ADA_CON2 & 0x1);
    short anc_gain = JL_ANC->CON5 & 0xFFFF;
    printf("MIC_G:%d,%d,MIC_6dB_EN:%d,%d, DAC_AG_MAX:%d,DAC_AG:%d,%d, DAC_DG_MAX:%d,DAC_DG:%d,%d, ANC_G:%d\n", mic0_gain, mic1_gain, mic0_6dB_en, mic1_6dB_en, dac_again_max, dac_again_l, dac_again_r, dac_dgain_max, dac_dgain_l, dac_dgain_r, anc_gain);
