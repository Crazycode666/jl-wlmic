...
static void anc_tone_play(u8 mode);
    ...
    while (1) {
        ...
        if (res == OS_TASKQ) {
            ...
            case ANC_MSG_TONE_SYNC:
                ...
                anc_tone_play(anc_hdl->param.mode);
...
static void anc_tone_play(u8 mode)
{
    u8 preemption = ANC_TONE_PREEMPTION;
    if (!bt_audio_is_running()) {	//后台无音乐播放时，提示音默认打断
        preemption = 1;
    }
    tone_play_index(anc_tone_tab[mode - 1], preemption);
}
