...
#if defined(TCFG_MUSIC_EFFECTS_EN) && TCFG_MUSIC_EFFECTS_EN
    void *effects_ultra;
#endif
#if (SYS_VOL_TYPE == VOL_TYPE_DIGITAL)
    dvol_handle *dvol;
...
extern int a2dp_media_get_content_protect_type();
...
static int get_rtp_header_len(u8 new_frame, u8 *buf, int len)
{
    ...
    if (new_frame) {
        header_len = byte_len + ext + 1 + a2dp_media_get_content_protect_type() ? 1 : 0;
    } else {
...
static void a2dp_stream_bandwidth_detect_handler(struct a2dp_dec_hdl *dec, int samples)
{
    ...
    if (max_latency < CONFIG_A2DP_ADAPTIVE_MAX_LATENCY) {
        ...
        if (a2dp_max_latency < CONFIG_A2DP_ADAPTIVE_MAX_LATENCY) {
...
#if defined(TCFG_MUSIC_EFFECTS_EN) && TCFG_MUSIC_EFFECTS_EN
        if (dec->effects_ultra) {
            music_effects_ultra_run(dec->effects_ultra, data, data, len);
        }
#endif

#if ((defined TCFG_EFFECT_DEVELOP_ENABLE) &&  TCFG_EFFECT_DEVELOP_ENABLE)
        if (dec->effect_develop) {
            audio_effect_develop_data_handler(dec->effect_develop, data, len);
        }
#endif
...
#if defined(TCFG_MUSIC_EFFECTS_EN) && TCFG_MUSIC_EFFECTS_EN
    a2dp_dec->effects_ultra = music_effects_ultra_open(dec->sample_rate, dec->ch);
#endif
...
#endif
#if defined(TCFG_MUSIC_EFFECTS_EN) && TCFG_MUSIC_EFFECTS_EN
    if (a2dp_dec->effects_ultra) {
        music_effects_ultra_close(a2dp_dec->effects_ultra);
        a2dp_dec->effects_ultra = NULL;
    }
#endif
...
int wdrc_get_filter_info_demo(void *_drc, struct audio_drc_filter_info *info)
{
#if (TCFG_AUDIO_BASS_BOOST && TCFG_DRC_ENABLE)
...
void a2dp_decoder_wdrc_filter_setup(struct a2dp_dec_hdl *dec, struct audio_fmt *fmt)
{
    ...
    drc_param.sr = dec->sample_rate;
    drc_param.channels = dec->ch;;
    drc_param.out_32bit = 0;
    drc_param.cb = wdrc_get_filter_info_demo;
    drc_param.name = AEID_MUSIC_BOOST_DRC;
    drc_param.cfg = &wdrc_p;
    drc_param.ver = 1;
    a2dp_wdrc = audio_dec_drc_open(&drc_param);
#endif/*TCFG_AUDIO_BASS_BOOST*/
}
