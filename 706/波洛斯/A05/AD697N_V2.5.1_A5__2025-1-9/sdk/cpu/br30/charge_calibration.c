...
static void charge_fix_vbg_deg(u8 vbg_aim, u8 vbat_aim)
{
...
static void charge_fix_vbg_inc(u8 vbg_aim, u8 vbat_aim)
{
    int vbg_t, vddio_t, vdc13_t, evdd_t, rvdd_t, dvdd_t;
...
static u8 charge_calibration_vbat_inc(struct calibration_data *p)
{
    ...
    p->div_vbat = (((u32)p->div_vbg * 15 * 4200) / 800) / 43;//43mV一个档位
    printf("cur_vbat: %d, div_vbat: %d\n", p->cur_vbat, p->div_vbat);
    ...
    printf("vbg_set: %d, vbat_set: %d\n", p->vbg_set, p->vbat_set);
...
static u8 charge_calibration_vbat_dec(struct calibration_data *p)
{
    ...
    p->div_vbat = (((u32)p->div_vbg * 15 * 4200) / 800 + 42) / 43;//43mV一个档位
    printf("cur_vbat: %d, div_vbat: %d\n", p->cur_vbat, p->div_vbat);
...
static u8 charge_calibration_bigger(u32 curr_err)
{
    ...
    if (p.cur_vbg >= p.div_vbg) {
        ...
        if (ret == CALIBRATION_CONTINUE) {
            charge_fix_vbg_deg(p.vbg_set, p.vbat_set);
            return CALIBRATION_CONTINUE;
        }
        /* return ret; */
        //尝试第三步
    ...
    if (p.cur_lev == CHARGE_mA_20) {
        ...
        if ((p.cur_vbg + p.div_vbg) <= 15) {
            ...
            if (ret == CALIBRATION_CONTINUE) {
                set_charge_mA(p.cur_lev);
                charge_fix_vbg_inc(p.vbg_set, p.vbat_set);
            }
        } else {
