...
extern u8 tuya_data[TUYA_TRIPLE_LENGTH];
...
int bt_tws_connction_status_event_handler(struct bt_event *evt)
{
    ...
    switch (evt->event) {
    case TWS_EVENT_CHANGE_TRIPLE:
#if TUYA_DEMO_EN
        printf("tws_change triple:");
        put_buf((u8 *)&tuya_data, TUYA_TRIPLE_LENGTH);
        const struct syscfg_ex_operations  *ex_ops = get_syscfg_btif_ex_ops();
        write_result = ex_ops->force_write(VM_TUYA_TRIPLE, (u8 *)&tuya_data, TUYA_TRIPLE_LENGTH - 2);
        if (write_result == TUYA_TRIPLE_LENGTH) {
...
static void play_tone_at_same_time(int tone_name, int err)
{
    ...
    case SYNC_TONE_POWER_OFF:
        ...
        /* bt_tws_poweroff(); */
        tone_play_index(p_tone->power_off, 1);
        break;
