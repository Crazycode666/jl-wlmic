...
#define CMD_BOX_TUYA_TRIPLE_READ    0x07 //获取涂鸦三元组
#define CMD_BOX_TUYA_TRIPLE_WRITE   0x08 //获取涂鸦三元组
...
typedef struct {
    u8  flag;
    u8  data[0x50];
} vm_tuya_triple_info_t;


#define TUYA_TRIPLE_LENGTH      0x3E
extern vm_tuya_triple_info_t triple_info;
typedef struct {
    u8  data[TUYA_TRIPLE_LENGTH];
} vm_tuya_triple;

vm_tuya_triple vm_triple;
extern int vm_read_result;
u8 succ = 0;
...
#if TUYA_TESTBOX_SYNC_EN
static u8 send_buf[70];
static u8 local_packet[70];
#else
...
#endif
extern const struct syscfg_ex_operataions *get_syscfg_btif_ex_ops(void);
...
void app_chargestore_testbox_sub_event_handle(u8 *data, u16 size)
{
    ...
    case CMD_BOX_CUSTOM_CODE:
        ...
        break;
#if TUYA_TESTBOX_SYNC_EN
    case CMD_BOX_TUYA_TRIPLE_WRITE:
        const struct syscfg_ex_operations  *ex_ops = get_syscfg_btif_ex_ops();
        send_buf[1] = CMD_BOX_TUYA_TRIPLE_WRITE;
        send_len = 3;
        memcpy((u8 *)&vm_triple, &data[1], size);
        if (succ == 0) {
            vm_write_result = ex_ops->force_write(VM_TUYA_TRIPLE, (u8 *)&vm_triple, TUYA_TRIPLE_LENGTH);
        }

        if (TUYA_TRIPLE_LENGTH == vm_write_result || (succ == 1) || vm_read_result == TUYA_TRIPLE_LENGTH) {
            printf("succ\n");
            succ = 1;
            send_buf[2] = TRIPLE_WRITE_SUCC;
        } else {
            printf("fail\n");
            send_buf[2] = TRIPLE_WRITE_FAIL;
...
void app_chargestore_testbox_sub_cmd_handle(u8 *buf, u8 len)
{
    ...
    case CMD_BOX_GLOBLE_CFG:
        ...
        chargestore_api_write(send_buf, 2);
        break;
    case CMD_BOX_TUYA_TRIPLE_READ:
#if TUYA_TESTBOX_SYNC_EN
        printf("read tuya triple:");
        memcpy(&send_buf[2], (u8 *)&triple_info, 1); //可以cpy一次
        printf("read_from:%d\n", send_buf[2]);
        send_len = 2 + 1 + TUYA_TRIPLE_LENGTH;
        put_buf((u8 *)&triple_info, 0x40);
        memcpy(&send_buf[2], (u8 *)&triple_info, TUYA_TRIPLE_LENGTH); //data +flag
        put_buf(send_buf, send_len);
        chargestore_api_write(send_buf, 0x3f);
#endif
        break;
    case CMD_BOX_TUYA_TRIPLE_WRITE:
#if TUYA_TESTBOX_SYNC_EN
        printf("rscv_write:");
        put_buf(buf, len);
        chargestore_event_to_user(&buf[1], DEVICE_EVENT_CHARGE_STORE, buf[0], len - 1);
        chargestore_api_set_timeout(200);
        break;
#endif
...
void app_chargestore_data_deal(u8 *buf, u8 len)
{
    ...
    if (app_charge_calibration_data_handler(buf, len)) {
        return;
    }
#endif
