...
static int mic_open(u8 *mic_gain, int sr)
{
    ...
    if (cvp_hdl->dut_mic_ch & (AUDIO_ENC_MPT_CVP_OUT | AUDIO_ENC_MPT_TALK_MIC | \
        ...
        /*单麦*/
#if TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_L
        mic_en[0] = 1;
        mic_id_tmp[0] = 1;
#elif TCFG_AUDIO_ADC_MIC_CHA & AUDIO_ADC_MIC_R
        mic_en[1] = 1;
        mic_id_tmp[1] = 1;
#endif /*TCFG_AUDIO_ADC_MIC_CHA*/
#endif /*TCFG_AUDIO_DUAL_MIC_ENABLE END*/
    ...
    if (cvp_hdl->dut_mic_ch & AUDIO_ENC_MPT_FF_MIC) {
        audio_enc_mpt_anc_mic_query(AUDIO_ADC_MIC_L, AUDIO_ENC_MPT_FF_MIC, mic_en, mic_id_tmp, mic_gain);
    }
    if (cvp_hdl->dut_mic_ch & AUDIO_ENC_MPT_FB_MIC) {
        audio_enc_mpt_anc_mic_query(AUDIO_ADC_MIC_R, AUDIO_ENC_MPT_FB_MIC, mic_en, mic_id_tmp, mic_gain);
    }
#endif/*TCFG_AUDIO_ANC_ENABLE*/
    ...
    if (cvp_hdl) {
        if (mic_en[0]) {
            cvp_hdl->mic_num++;
            audio_adc_mic_open(&cvp_hdl->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);
            audio_adc_mic_set_gain(&cvp_hdl->mic_ch, mic_gain[0]);
        }
        if (mic_en[1]) {
            cvp_hdl->mic_num++;
            audio_adc_mic1_open(&cvp_hdl->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);
...
int audio_enc_mpt_cvp_open(u16 mic_ch)
{
    ...
    mic_gain[1] = app_var.aec_mic_gain;
    mic_gain[2] = app_var.aec_mic_gain;
    mic_gain[3] = app_var.aec_mic_gain;
...
REGISTER_LP_TARGET(audio_enc_mpt_cvp_lp_target) = {
    .name = "audio_cvp",
