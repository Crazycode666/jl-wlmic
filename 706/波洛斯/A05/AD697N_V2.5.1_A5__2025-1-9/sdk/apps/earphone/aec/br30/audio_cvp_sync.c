...
#define AUDIO_CVP_SRCBUF_SIZE	640		//输入输出BUFFER长度
...
typedef struct {
    ...
    u32 sample_rate;					//采样率
    u8 src_inbuf[AUDIO_CVP_SRCBUF_SIZE];
    u8 src_outbuf[AUDIO_CVP_SRCBUF_SIZE];
...
{
    if (hdl->hw_src) {
        audio_hw_src_stop(hdl->hw_src);	//stop 和 close 有什么区别
