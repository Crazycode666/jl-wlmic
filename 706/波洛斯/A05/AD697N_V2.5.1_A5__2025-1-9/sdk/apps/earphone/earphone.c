...
void sys_enter_soft_poweroff(void *priv)
{
    ...
    log_info("%s, %d\n", __func__, (int)priv);
    tone_play_index(IDEX_TONE_POWER_OFF, 1);
...
{
    extern bool get_hfp_connect_finish(u8 * addr);
    u8 *get_cur_connect_phone_mac_addr(void);
    if (get_hfp_connect_finish(get_cur_connect_phone_mac_addr())) {
        log_info(">>>>wail hfp connect\n");
        sys_timeout_add(NULL, bt_get_time_date, 200);
    } else {
