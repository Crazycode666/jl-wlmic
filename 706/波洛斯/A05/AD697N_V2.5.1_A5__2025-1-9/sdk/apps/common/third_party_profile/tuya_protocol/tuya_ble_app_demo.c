...
void a2dp_state_change_handler(uint8_t status)
{
    printf("a2dp_state_change_handler, status:%d", status);
...
void tuya_key_info_indicate()
{
    ...
    for (int key_idx = 0; key_idx < 6; key_idx++) {
        ...
        p_dp_data.type = TUYA_SEND_DATA_TYPE_DT_STRING;
        p_dp_data.len = U16_TO_LITTLEENDIAN(1);
        switch (key_idx) {
        case 0:
...
void tuya_app_cb_handler(tuya_ble_cb_evt_param_t *event)
{
    //OS_ENTER_CRITICAL();
    int16_t result = 0;
    //printf("tuya cb_handler event->evt=0x%x, task:%s\n", event->evt, os_current_task());
    switch (event->evt) {
