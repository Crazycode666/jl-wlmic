...
#if TCFG_LOUDNESS_DEBUG && WIRELESS_MIC_LOUDNESS_DEBUG
#define PRINTF_LOUNDNESS_TIMER   (100)  //MS  100ms打印一次最大值
#define LOUDNESS_THREAD_dB       (-30)  //100ms内大于 该值就打印一次
...
static void peak_dB_cal_ctrl(struct peak_dB *hdl, void *data, int len, int ch_num)
{
    ...
    if (ch_num == 2) {
        ...
        dB = peak_dB_cal(mono_pcm, len / 2, BIT_WIDE_16BIT);
        free(mono_pcm);
        mono_pcm = NULL;
    } else {
        mono_pcm = data;
        dB = peak_dB_cal(mono_pcm, len, BIT_WIDE_16BIT);
    }
...
{
    struct live_mic_effect *effect = (struct live_mic_effect *)priv;
    if (!effect) {
        return 0;
    }
    peak_dB_cal_ctrl(&effect->loudness_in, in->data, in->data_len, in->channel);
    return in->data_len;
}
#endif
...
#if WIRELESS_MIC_CONFIG_AGC
static agc_param_t agc_parm = {
    .AGC_max_lvl = 0,
    .AGC_fade_in_step = 1.3f,
    .AGC_fade_out_step = 1.3f,
    .AGC_max_gain = 12.f,
    .AGC_min_gain = 0,
    .AGC_speech_thr = -50.f,
    .AGC_samplerate = 48000,
    .AGC_frame_size = 120, //帧长(short)，处理数据长度必须是帧长的倍数
};
#endif
...
#if TCFG_LOUDNESS_DEBUG && WIRELESS_MIC_LOUDNESS_DEBUG
    effect->loudness_in.index = 0;
    effect->loudness_out.index = 1;
    effect->loudness_debug_hdl = stream_entry_open(effect, audio_data_check_cb, 0);// 能量计算及打印节点
#endif

//GAIN节点
#if TCFG_MIC_GAIN_ENABLE && WIRELESS_MIC_CONFIG_GAIN
    struct aud_gain_parm wireless_mic_gain_parm = {
        .gain[0] = mic_eff->gain_parm.parm.gain[0],
        .gain[1] = mic_eff->gain_parm.parm.gain[1],
        .gain[2] = mic_eff->gain_parm.parm.gain[2],
        .gain[3] = mic_eff->gain_parm.parm.gain[3],
        .channel = ch_num,
        .indata_inc = (ch_num == 1) ? 1 : 2,
        .outdata_inc = (ch_num == 1) ? 1 : 2,
        .gain_name = AEID_MIC_GAIN,
    };
    effect->gain = audio_gain_process_open(&wireless_mic_gain_parm);
#endif
...
// REVERB节点
    ...
    //AGC节点
#if WIRELESS_MIC_CONFIG_AGC
    agc_parm.AGC_samplerate = effect->param.sample_rate;
    effect->agc = adapter_stream_agc_open(&agc_parm);
#endif
...
#if WIRELESS_MIC_CONFIG_AGC
    if (effect->agc) {
        entries[entry_cnt++] = &effect->agc->stream->entry;
    }
#endif
...
#if defined(SYS_DIGVOL_GROUP_EN)&&SYS_DIGVOL_GROUP_EN&&WIRELESS_MIC_TX_EFFECT_ENABLE
    void *dvol_entry = sys_digvol_group_ch_open("music_live_capture", -1, NULL);
...
void reverb_onff()
{
    ...
    /*if (g_mic_effect->sub_0_plate_reverb_hdl) {
        pause_echo(g_mic_effect->sub_0_plate_reverb_hdl,mark);
    }*/
    close_echo(g_mic_effect->sub_0_plate_reverb_hdl);
}
...
#if WIRELESS_MIC_CONFIG_AGC
    if (effect->agc) {
        adapter_stream_agc_close(&effect->agc);
    }
#endif
...
#if TCFG_MIC_GAIN_ENABLE && WIRELESS_MIC_CONFIG_GAIN
    if (effect->gain) {
        audio_gain_process_close(effect->gain);
    }
#endif
