...
struct low_power_param {
    ...
    u8 btosc_disable;
    u8 delay_us;
    u8 soff_mode;			//1: close vddio mode 0: keep vddio mode
};
...
struct boot_soft_flag_t {
    ...
    union {
        struct soft_flag1_t usb_io;
        u8 value;
    } flag1;
    union {
        struct soft_flag2_t pa5_pa6;
        u8 value;
    } flag2;
    union {
        struct soft_flag3_t pp0_io;
        u8 value;
    } flag3;
};
...
//设置系统默认电源模式
void power_set_default_mode(u8 mode);
u8 is_extern_dcdc(void);

//设置系统实时电源模式
void power_set_current_mode(u8 mode);

u8 power_get_current_mode(void);
